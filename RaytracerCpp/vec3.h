...
class vec3 {
	public:
		...
		bool near_zero() const {
			auto nearzero = 1e-8;
			return (fabs(e[0]) < nearzero) && (fabs(e[1]) < nearzero) && (fabs(e[2]) < nearzero);
		}
...
inline vec3 random_in_himisphere(const vec3& normal)
{
	vec3 on_sphere = random_unit_vector();
	return (dot(normal, on_sphere) > 0.0) ? on_sphere : -on_sphere;
}
